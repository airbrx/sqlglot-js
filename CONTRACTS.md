...
| `Generator.__init__` | 12 kwargs become one options object | JS has no keyword arguments. Same precedent as `TokenizerCore.__init__`'s 26 and `helper.csv`. Field order is upstream's parameter order. |
| `Generator.__init__`'s `Dialect.get_or_raise` | injected; a dialect NAME **throws** | `dialects/dialect.js` lands at P5. Identical rule and rationale to the `Tokenizer.__init__` row above: silently falling back to the default dialect would make every per-dialect generate-oracle row vacuously green. Base-`Dialect` defaults live in `BASE_DIALECT_GENERATOR_SETTINGS`, read out of CPython at the pin, and a field a stub needs later must be added there from CPython rather than guessed (R18(d)). |
| `Generator.func` / `format_args` | keyword-only args after `*args` become a trailing options object | Upstream signatures are `func(name, *args, prefix="(", suffix=")", normalize=True)` and `format_args(*args, sep=", ")`. Exactly the `_match_text_seq` shape R18 records: passing one positionally would silently make it another **argument to render**. |
| `Generator.expressions` / `indent` / `maybe_comment` | trailing options object, though the params are ordinary Python defaults | Not forced by the language — chosen on measurement. Across upstream's own 169 / 14 / 7 call sites, **not one** passes an argument beyond the 2nd positionally; every other argument is already a keyword. A 12-parameter positional JS signature would be the deviation from how the code is actually written, and is the shape most likely to produce R18-class call sites in the 426 stubs still to be ported. `sql()` is deliberately **not** in this list: 579 of its 708 call sites pass `(expression, key)` positionally, so it stays positional. |
| `Generator._buildDispatch` | plain function called from the constructor, not a class-creation hook | JS has no metaclass hook — but none is needed, because upstream does not build at class-creation time either: it builds lazily in `__init__` and memoises in `_DISPATCH_CACHE`. The prototype-chain walk plus an explicit sort reproduces `dir(cls)`'s MRO span and sorted order. Values are stored as method NAMES (resolved on the instance at call time, so subclass overrides win) rather than bound functions, which also makes the table directly comparable against `_gen/dispatch/`. |
| `Generator.SUPPORTED_JSON_PATH_PARTS` | a **throwing static getter**, not a value | Its upstream value is `set(JSON_PATH_PART_TRANSFORMS)` from the unported `jsonpath.py`. An empty `Set` would make every membership test silently return false; absence would read back `undefined` and surface as an output mismatch far from the cause (R18's `REGEXP_EXTRACT_DEFAULT_GROUP`). Throwing keeps the name present for §4.2's field-set check, fails at first read, and stays greppable. Populate it **derived from** `jsonpath.js`'s table when that lands, never as a hand-written class list. |
...
| a dialect's nested `class Tokenizer(tokens.Tokenizer)` / `class Generator` / `class Parser` | a module-level class assigned to a `static Tokenizer` / `Parser` field | JS has no nested class declaration. `registerDialect` looks for the field name (py:291's `klass.__dict__.get("Tokenizer", ...)`), so the field name is upstream's and only the CLASS name differs (`SnowflakeTokenizer` vs upstream's `Tokenizer`). `klass.name` is already non-contractual here — see the `_Dialect` registration row above for why the registry key is a parameter. Consequence worth stating: because the nested class is created outside any hook, its `initTokenizerSubclass(cls)` call is the porter's responsibility, and omitting it leaves every derived value (`_KEYWORD_TRIE`, `_COMMENTS`, `_FORMAT_STRINGS`, `_STRING_ESCAPES`) inherited from the BASE tokenizer — a total failure of the dialect's lexer that still parses most SQL. `spike/p5/fuzz_snowflake_dialect.mjs` diffs all six derived values against CPython for exactly that reason. |
| `exp.SAFE_IDENTIFIER_RE` (`re.compile(r"^[_a-zA-Z][\w]*$")`) | a frozen object with a `.test()` method, not a `RegExp` | two independent cross-language differences in one four-token pattern. (1) Python's `\w` on a `str` pattern is UNICODE-aware — `str.isalnum()` plus underscore — while JS `\w` is *always* `[A-Za-z0-9_]`; the `u` flag does not change it (it governs `\p{...}` and surrogates). `"Straße"` and `"DUAＬ"` are safe identifiers upstream. Equivalence to `pyIsAlnum(ch) || ch === '_'` is established by a full `sys.maxunicode` sweep with **0 mismatches over 1,114,112 code points**, per §4.6, so no runtime `\p{...}` is needed (`tools/lint_unicode.mjs` forbids it). (2) Python's `$` matches before a single trailing newline, so `"abc\n"` matches and `"abc\n\n"` does not; JS `$` without `m` matches only at the very end. Kept under upstream's name with a `.test()` method so both call sites (`toIdentifier`'s `quoted` default, `Dialect.can_quote`) stay byte-identical transliterations. Not reachable from `corpus/ast/` — the divergence needs an ASCII first character followed by a non-ASCII word character and no atom has one — so `test/expressions/core.test.mjs` carries the regression guard. |
