...
- Corrected native upstream scope (PORT_PLAN Q6): all six parser/generator-independent tests are hand-ported and green. The test file carries the explicit list of all 65 P4-deferred names.
- `DateAdd` kwargs construction normalizes `day` to `DAY`; `astLoad` deliberately bypasses the init hook and preserves `day`.
...
Native tests also cover traversal, mutation, transforms, datatype/cast behavior, query/DDL/DML builders, focused class constants, collection projections, comments/meta, parent/index maintenance and Pivot output columns: **19/19 tests green**.
