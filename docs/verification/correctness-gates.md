...
> Integration update: current main includes simplify and the review fixes. The strict
> full suite now has TWO red diagnostics (AST parity and full base-output generation).
> Neither is passed off as release-green; all production rows remain in ID ratchets.
...
|---|---|
| `native-tests` | All native tests, including production regression injection: 512 pass, 0 fail |
| `lints` | All seven Makefile lints, including anchors and ported-range deny gate |
| `corpus-ratchet` | Real parse/generate/warning/error adapter; protect 5,688 accepted atom IDs; no deletions or changed expectations |
...
| Pinned AST → dialect SQL + warnings | 6,929 | 718 | 7,592 | 301 | 0 | 15,540 |
...
## Fresh full-suite triage: ten initial red groups; two strict full-parity diagnostics remain after review
...
bash spike/run_all.sh                # diagnostic: exit 1, strict AST and base-output gaps
...
## Merge-review correction: base generation cannot depend on the reader

PR #76's P1 review found that AST-fed base generation still excluded foreign read
keys, even though no parser was called. Removed that guard and added a native
regression test deriving the required denominator from the corpus's output dialect.
It now evaluates **1,182** base-output rows, not225: **887 exact, 8 mismatch,
287 stub, 0 other error**, and exits1. The older demand-informed diagnostic classifies
50 missing TRANSFORMS as stubs; the production generation report deliberately calls
those **ERROR**, yielding887 PASS /8 MISMATCH /237 STUB /50 ERROR for the same base
population. Neither classification drops rows. The remaining14,358 non-base OUTPUT
rows are outside this one diagnostic, but ALL are in the required full generation gate.

Do not make this newly exposed mismatch population disappear to regain a green
strict base probe. It stays in run_all.sh, visibly red. Required generation coverage
comes from the complete production pass-ID ratchet, including every accepted
base-output pass; the strict diagnostic is not an additional merge prerequisite.
Its154 demand-predicted rows still have zero overclaims. No runtime source or
expectation was changed to make these eight SQL mismatches pass.

Main's already-landed simplify prerequisites unlock twelve generation-only exact
IDs (the eleven end-to-end IDs listed in corpus-baseline.md plus
`2dbfbc38e0e38776`, whose parse path remains unported). Reviewed these against the
unchanged AST/output references, added only those twelve IDs, and removed none.
Generation is now6,929 exact /718 mismatch /7,592 stub /301 error, total15,540,
exclusions0. Parse remains10,279 exact /95 mismatch /5,098 stub /68 error.
The new simplify oracle is included in the required applicable oracle suite; both
