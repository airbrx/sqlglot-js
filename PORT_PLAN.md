...
**R15 — full Snowflake AST-oracle exactness is not reachable from base `parser.js` alone; it needs `dialects/snowflake.py`'s parser-specific overrides, a component this project has never scoped or started.** Found while chasing R14's fix toward `spike/run_all.sh` green: after every reachable base-parser bug was fixed (R13, R14, and the round above), Snowflake closure plateaued at 975/2,516 exact (38.8%). Triaging the remaining 344 Snowflake `ERROR` rows by construct found the large majority are `SHOW ...` (64), stage references `@...` (27), policy/tag DDL (15), `COPY INTO`/`PUT`/`GET` (12), `FLATTEN` (8), `IDENTIFIER(...)` (7), `RELY`/`NORELY` (4), plus dialect-specific function-name mappings (`TRY_TO_DATE`, `ZEROIFNULL`, `REGEXP_SUBSTR`, `TO_TIMESTAMP`, ...) that dominate the 1,111 Snowflake `MISMATCH` rows — all confirmed to live in `sqlglot/dialects/snowflake.py`, not base `sqlglot/parser.py`. This is a real, separate ~2,000+ LOC component (the gap between P3's 11,920 LOC budget and base `parser.py`'s own ~9,800 LOC, and the literal second half of P3's own title, "Parser base + **Snowflake parser**") that no PR or stub-queue dispatch has ever targeted — `src/dialects/snowflake.js` does not exist yet, the same way `src/dialects/dialect.js` doesn't (R13's `VALID_INTERVAL_UNITS` note). *Not the same as the already-documented "100% of `test_snowflake.py` only at P9" fact (§3.2, R3)* — that one is about atoms whose *write* dialect isn't yet implemented; this one is about Snowflake's own *read*-side grammar. *Decision (Ben, 2026-09-03):* merge the real, verified base-parser progress now rather than block on scoping and building the new dialect-parser component in the same pass — "let's merge and move on." `dialects/snowflake.js`'s parser portion is tracked here as explicitly deferred, not silently dropped; whoever picks it up next should start from this paragraph's construct list rather than re-triaging from scratch. Until it lands, `spike/run_all.sh`'s "P3: AST oracle coverage (honest)" probe is expected to stay red — that is not a regression signal, it is this exact gap, and should not be "fixed" by loosening the probe's own MISMATCH=ERROR=0 standard.
