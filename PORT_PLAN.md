...
- `corpus/deny/operators.json` — **68 operator-overload sites inside `sqlglot/` itself.** e.g. `dialect.py:1778` builds AST with `exp.Length(...) - exp.paren(expression.expression - 1)`; `((value - 1) // 32768) + 1` on an `exp.Expr` builds nodes, not numbers. The agent contract ("transliterate; never use `Number` for SQL literals") *actively produces wrong code* at these exact lines, which is why they must be listed rather than reasoned about.
- `corpus/deny/implicit_str.json` — 26 implicit `__str__` sites that render with the default dialect.
...
**Measured on this box (Node v22.12.0):** a trivial frame overflows at depth **9,160**; a frame resembling a real generator method (5 args, several locals, string building) overflows at **4,225**.
...
**R5 — 68 operator-overload sites and 26 implicit-`__str__` sites inside `sqlglot/` itself.** The agent contract actively produces wrong code at those exact lines. *Mitigation:* machine-generated deny-lists + CI lint, now extended to Python builtins (`corpus/deny/py_builtins.json`). *Residual:* the census derives from `tests/dialects` coverage, so a site reachable only from an unported dialect could be missed; the census re-runs on every bump.
